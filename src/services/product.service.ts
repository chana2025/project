...
export const productService = {
    ...
    downloadProductsExcel: async (): Promise<void> => {
        try {
            const response = await axios.get(`${API_BASE_URL}/Product/download-excel`, {
                responseType: 'blob', // חשוב להורדת קבצים בינאריים
            });
    
            // יצירת כתובת זמנית לקובץ
            const url = window.URL.createObjectURL(new Blob([response.data]));
    
            // יצירת קישור זמני
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', 'Products.xlsx'); // שם הקובץ שיורד
            document.body.appendChild(link);
            link.click();
            ...
            console.error("שגיאה בהורדת קובץ Excel של מוצרים:", error);
            throw error;
        }
    }
};