...
//     // Effect לטעינת דיאטות
//     useEffect(() => {
//         const fetchDiets = async () => {
//             try {
//                 setLoadingDiets(true);
//                 const fetchedDiets = await dietService.getAllDiets();
//                 setDiets(fetchedDiets.slice(0, 3)); // הצג רק 3 דיאטות ראשונות
//             } catch (error) {
//                 console.error("Error fetching diets:", error);
...
//     // פונקציה לטיפול בבחירת מוצר מחיפוש
//     const handleProductSelect = async (prod: { id: number; name: string }) => {
//         setSelectedProduct(null); // נקה מוצר קודם
//         setErrorProductDetails(null); // נקה שגיאות קודמות
//         setLoadingProductDetails(true); // הצג מצב טעינה
//         try {
...
//     // פונקציה לטיפול בהורדת קובץ אקסל
...
//                         <h4>{selectedProduct.Name}</h4> {/* Name נשאר עם N גדולה כי זה בדרך כלל שם המוצר */}
//                         <ul>
//                             {/* **תיקון כאן**: שימוש בשמות המאפיינים כמו ב-ProductType.types.ts (אות קטנה) */}
//                             {/* וגם הוספת key ייחודי לכל li */}
//                             <li key="calories">קלוריות: {selectedProduct.Calories} קק"ל</li>
//                             <li key="protein">חלבון: {selectedProduct.Protein} גרם</li>
//                             <li key="fat">שומן: {selectedProduct.Fat} גרם</li>
//                             <li key="carbohydrates">פחמימות: {selectedProduct.Carbohydrates} גרם</li>
//                             <li key="source">מקור: {selectedProduct.SourceApi || 'לא ידוע'}</li> {/* שימוש ב-sourceApi והוספת "לא ידוע" אם הוא null */}
//                         </ul>
//                     </div>
//                 )}
//             </section>
    ...
    return (
        <div className="home-page">
            ...
            <section className="product-search-section container">
                ...
                {selectedProduct && (
                    <div className="product-details">
                        {/* ✅ תיקון: שימוש ב-selectedProduct.name (עם n קטנה) */}
                        <h4>{selectedProduct.name}</h4> 
                        <ul>
                            {/* ✅ תיקון: שימוש בשמות המאפיינים כמו ב-ProductType (camelCase) */}
                            <li key="calories">קלוריות: {selectedProduct.calories} קק"ל</li>
                            <li key="protein">חלבון: {selectedProduct.protein} גרם</li>
                            <li key="fat">שומן: {selectedProduct.fat} גרם</li>
                            <li key="carbohydrates">פחמימות: {selectedProduct.carbohydrates} גרם</li>
                            {/* ✅ תיקון: שימוש ב-selectedProduct.sourceApi */}
                            <li key="source">מקור: {selectedProduct.sourceApi || 'לא ידוע'}</li> 
                        </ul>
                    </div>
                )}
            </section>
